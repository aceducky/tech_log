...
export function responseRulesPrompt(): string {
  return [
    ...
    "When answering from a log, cite it as: Source: [Title](/logs/{slug}). Mention the author when relevant. Do not dump the entire raw markdown unless the user asks for it - summarize and quote the relevant section.",
    "If you are citing the same log the user is currently viewing, do not add a link to it - they are already reading it. Only cite other logs with links.",
